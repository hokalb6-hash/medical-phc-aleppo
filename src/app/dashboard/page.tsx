...
export default async function DashboardPage() {
  ...
  const centerFilter = profile.role === "super_admin" ? undefined : profile.center_id;

  const getCounts = unstable_cache(
    async () => fetchDashboardCounts(centerFilter ?? undefined),
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {cards.map((card) => (
          <div key={card.label} className="surface-card p-4">
            ...
            <p className="mt-2 text-3xl font-bold text-blue-800">{card.value}</p>
          </div>
        ))}
      </div>
    </div>
